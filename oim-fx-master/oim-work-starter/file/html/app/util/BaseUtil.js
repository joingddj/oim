...
function replaceToHtml(text) {
    ...
    if (!isEmpty(text) && type === 'string') {
        let value = '';
        for (let i = 0; i < text.length; i++) {
            let c = text[i].charCodeAt();
            if (text[i] === '&') {
                value += text[i];
                continue;
            }
...
function toDate(text) {
    ...
    let date = new Date();
    if (text.length === 2) {
        let yymmdd = text[0].split('-');
        let hhmmss = text[1].split(':');
...
function getDateInterval(interval, date1, date2) {
    let TimeCom1 = new TimeData(date1);
    let TimeCom2 = new TimeData(date2);
    let result;
...
Array.prototype.in_array = function (e) {
    let index = this.getIndexByProperty('', e);
    if (index > -1) {
        return true;
    }
    return false;
...
Array.prototype.in_arrayById = function (e) {
    let index = this.getIndexByProperty('id', e);
    if (index > -1) {
        return true;
    }
    return false;
...
Array.prototype.removeByProperty = function (property, value) {
    let index = this.getIndexByProperty(property, value);
    if (index > -1) {
        this.splice(index, 1);
    }
};
...
function openWindow(url, name, width, height) {
    ...
    let wd = window.open(url, name, 'height=' + height + 'px,width=' + width + 'px,top=' + top + 'px,left=' + left + 'px,resizable=yes,toolbar:0,menubar:0,scrollbars=1');
    return wd;
}
