 ...
 */
package com.oimchat.app.fx.view.ui.classics.module.account;
 ...
 * @author Only
 */
public class RegisterStage extends ClassicsCommonStage {
	...
	private OnlyPopupOver accountOver = new OnlyPopupOver();// 昵称提示
	...
	private void initComponent() {
		...
		accountField.setPromptText("账号(字母+数字)");
		...
		accountLabel.setPrefSize(55, 25);
		accountLabel.setLayoutX(10);
		accountLabel.setLayoutY(20);
		accountField.setPrefSize(290, 25);
		accountField.setLayoutX(accountLabel.getLayoutX() + accountLabel.getPrefWidth() + 10);
		accountField.setLayoutY(accountLabel.getLayoutY());

		nicknameLabel.setPrefSize(55, 25);
		nicknameLabel.setLayoutX(10);
		nicknameLabel.setLayoutY(accountField.getLayoutY() + accountField.getPrefHeight() + 15);
		...
		infoPane.getChildren().add(accountLabel);
		...
		infoPane.getChildren().add(accountField);
		...
		waitingPanel.add(WaitingPane.show_waiting, "注册...", ImageLoadUtil.getImageByClassPath("/general/view/common/images/loading/loading_bar_312x4.gif"));
	...
	public boolean verify() {
		...
		if (null == account || "".equals(account)) {
			accountOverLabel.setText("请输入您账号(用户名)!");
		...
		if (null == nickname || "".equals(nickname)) {
			...
			nicknameOver.show(passwordField);
			if (mark) {
	...
	public void showAccountPopup(String text) {
		accountOverLabel.setText(text);
		accountOver.show(accountField);
		accountField.requestFocus();
	}
