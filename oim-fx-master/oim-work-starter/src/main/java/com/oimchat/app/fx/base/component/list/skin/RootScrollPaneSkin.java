...
public class RootScrollPaneSkin extends BehaviorSkinBase<ScrollPane, ScrollPaneBehavior> implements TraverseListener {
	/***************************************************************************
	 * * UI Subcomponents * *
	 **************************************************************************/
	...
	public RootScrollPaneSkin(final ScrollPane scrollpane) {
		...
		public void invalidated(Observable valueModel) {
			if (!nodeSizeInvalid) {
				...
				if (vsbvis != determineVerticalSBVisible() || hsbvis != determineHorizontalSBVisible() ||
					...
					/**
					 * we just need to update scrollbars based on new scrollNode size, but we don't
					 * do this while dragging, there's no need, and it jumps, as dragging updates
					 * the scrollbar too.
					 */
					if (!dragDetected) {
						updateVerticalSB();
	...
	private final ChangeListener<Bounds> boundsChangeListener = new ChangeListener<Bounds>() {
		...
		public void changed(ObservableValue<? extends Bounds> observable, Bounds oldBounds, Bounds newBounds) {
			...
			/*
			 ** For a height change then we want to reduce viewport vertical jumping as much
			 * as possible. We set a new vsb value to try to keep the same content position
			...
			/*
			 ** For a width change then we want to reduce viewport horizontal jumping as much
			 * as possible. We set a new hsb value to try to keep the same content position
...
//        setManaged(false);
		...
		viewContent = new StackPane() {
			...
			public void requestLayout() {
				...
				RootScrollPaneSkin.this.getSkinnable().requestLayout();
			}
		...
		viewRect.addEventFilter(MouseEvent.MOUSE_RELEASED, e -> {
			...
			/*
			 ** if the contents need repositioning, and there's is no touch event in
			 * progress, then start the repositioning.
			 */
		...
		viewRect.setOnMouseDragged(e -> {
			...
			/*
			 ** we need to consume drag events, as we don't want the scrollpane itself to be
			 * dragged on every mouse click
			 */
			e.consume();
		});

		/*
		 ** don't allow the ScrollBar to handle the ScrollEvent, In a ScrollPane a
		 * vertical scroll should scroll on the vertical only, whereas in a horizontal
		 * ScrollBar it can scroll horizontally.
		 */
		...
		/*
		 * listen for ScrollEvents over the whole of the ScrollPane area, the above
		 * dispatcher having removed the ScrollBars scroll event handling.
		 *
		 * Note that we use viewRect here, rather than setting the eventHandler on the
		 * ScrollPane itself. This is for RT-31582, and effectively allows for us to
		 * prioritise handling (and consuming) the event internally, before it is made
		 * available to users listening to events on the control. This is consistent
		 * with the VirtualFlow-based controls.
		 */
		viewRect.addEventHandler(ScrollEvent.SCROLL, event -> {
			...
			/*
			 ** if we're completely visible then do nothing.... we only consume an event that
			 * we've used.
			 */
			if (vsb.getVisibleAmount() < vsb.getMax()) {
				...
				if (!IS_TOUCH_SUPPORTED) {
					...
					/*
					 ** if there is a repositioning in progress then we only set the value for 'real'
					 * events
					 */
		...
		/*
		 ** there are certain animations that need to know if the touch is happening.....
		 */
		...
		// ScrollPanes do not block all MouseEvents by default, unlike most other UI
		// Controls.
	...
	void scrollBoundsIntoView(Bounds b) {
		...
		if (dx != 0) {
			...
			// Adjust back for some amount so that the Node border is not too close to view
			// border
	...
	/**
	 * Computes the size that should be reserved for horizontal scrollbar in size
	 * hints (min/pref height)
	 */
	...
	protected void layoutChildren(final double x, final double y,
		...
		if (vsbvis) {
			 ...
			 **
			 ** Positioning the ScrollBar The Padding should go between the content and the
			 * edge, otherwise changes in padding move the ScrollBar, and could in extreme
			 * cases size the ScrollBar to become unusable. The -1, +1 plus one bit : If
			 * padding in => 1 then we allow one pixel to appear as the outside border of
			 * the Scrollbar, and the rest on the inside. If padding is < 1 then we just
			 * stick to the edge.
			 */
	...
	private boolean determineHorizontalSBVisible() {
		...
		if (IS_TOUCH_SUPPORTED) {
			...
			// RT-17395: ScrollBarPolicy might be null. If so, treat it as "AS_NEEDED",
			...
			return (ScrollBarPolicy.NEVER == hbarPolicy) ? false
					: ((ScrollBarPolicy.ALWAYS == hbarPolicy) ? true
							: ((sp.isFitToWidth() && scrollNode != null ? scrollNode.isResizable() : false) ? (nodeWidth > contentWidth && scrollNode.minWidth(-1) > contentWidth) : (nodeWidth > contentWidth)));
		}
	}
		...
		if (IS_TOUCH_SUPPORTED) {
			...
			return (ScrollBarPolicy.NEVER == vbarPolicy) ? false
					: ((ScrollBarPolicy.ALWAYS == vbarPolicy) ? true
							: ((sp.isFitToHeight() && scrollNode != null ? scrollNode.isResizable() : false) ? (nodeHeight > contentHeight && scrollNode.minHeight(-1) > contentHeight) : (nodeHeight > contentHeight)));
		}
	}
	...
	private void computeScrollBarSize() {
		...
		if (vsbWidth == 0) {
			// println("*** WARNING ScrollPaneSkin: can't get scroll bar width, using
			// {DEFAULT_SB_BREADTH}");
			if (IS_TOUCH_SUPPORTED) {
		...
		if (hsbHeight == 0) {
			// println("*** WARNING ScrollPaneSkin: can't get scroll bar height, using
			// {DEFAULT_SB_BREADTH}");
			if (IS_TOUCH_SUPPORTED) {
	...
	protected void startSBReleasedAnimation() {
		if (sbTouchTimeline == null) {
			/*
			 ** timeline to leave the scrollbars visible for a short while after a
			 * scroll/drag
			 */
		...
		if (contentPosX == null) {
			contentPosX = new DoublePropertyBase() {
				...
				public Object getBean() {
					return RootScrollPaneSkin.this;
				}
