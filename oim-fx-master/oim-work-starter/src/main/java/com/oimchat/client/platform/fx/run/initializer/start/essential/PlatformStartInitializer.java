...
import com.oimchat.client.platform.fx.basic.constant.FxAppConstant;
...
public class PlatformStartInitializer implements StartInitializer {

	@Override
	public void initialize(AppContext appContext) {
		initAppInfo(appContext);
		SystemUnit su = appContext.getObject(SystemUnit.class);
		su.addExitExecuteAction(() -> {
			FxUtil.invoke(() -> {
				Platform.exit();
				System.exit(0);
			});
	...
	private void initAppInfo(AppContext appContext) {
		AppInfoBox box = appContext.getObject(AppInfoBox.class);
		box.getAppInfo().setBuild(FxAppConstant.build);
		box.getAppInfo().setName(FxAppConstant.name);
		box.getAppInfo().setType(FxAppConstant.type);
		box.getAppInfo().setVersion(FxAppConstant.version);
		box.getAppInfo().setBuild(FxAppConstant.build);

		AppEnvInfo info = appContext.getObject(AppEnvInfo.class);
		info.setBuild(FxAppConstant.build);
		info.setName(FxAppConstant.name);
		info.setType(FxAppConstant.type);
		info.setVersion(FxAppConstant.version);
		info.setBuild(FxAppConstant.build);

		info.setHome(AppConstant.home);
	}
}
