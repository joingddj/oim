...
package com.oimchat.client.platform.common.run.initializer.enter.essential;
...
import com.onlyxiahui.app.initializer.EnterInitializer;

/**
 * Description <br>
 * Date 2020-11-19 14:07:07<br>
 * 
 * @author XiaHui [onlovexiahui@qq.com]<br>
 * @since 1.0.0
 */

public class PlatformLogInitializer implements EnterInitializer {

	@Override
	public void initialize(AppContext appContext) {
		PathEnv pe = appContext.getObject(PathEnv.class);
		try {
			System.setProperty("logFilePath", pe.getLogFilePath());
			// 获取日志配置上下文
			final LoggerContext ctx = (LoggerContext) LogManager.getContext(false);
			ctx.reconfigure();
//			refreshLoggingPath(pe.getLogFilePath());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void refreshLoggingPath(String logPath) {
		// 获取日志配置上下文
		final LoggerContext ctx = (LoggerContext) LogManager.getContext(false);
		...
		String outPattern = "%highlight{%d{yyyy-MM-dd HH:mm:ss.SSS} [%t] %-5level %logger{36} : %msg%n}{%throwable{short.fileName}}{FATAL=white, ERROR=red, WARN=blue, INFO=black, DEBUG=green, TRACE=blue}";
		String filePattern = logPath + "/$${date:yyyy-MM}/" + appName + "-%d{yyyy-MM-dd-HH}-%i.gz";
		String name = "fileOut";
		String fileName = logPath + "oim.log";
		PatternLayout patternLayout = PatternLayout.newBuilder().withPattern(outPattern).build();
		Filter[] filters = new Filter[] {
				...
				ThresholdFilter.createFilter(Level.ERROR, Result.ACCEPT, Result.DENY) };

		filters = new Filter[] {};

		org.apache.logging.log4j.core.Filter filter = CompositeFilter.createFilters(filters);
		DefaultRolloverStrategy strategy = DefaultRolloverStrategy.newBuilder().withMax("50").build();
		...
		RollingRandomAccessFileAppender fileOut = RollingRandomAccessFileAppender.newBuilder()
				.setConfiguration(config)
				.withFileName(fileName)
				.withFilePattern(filePattern).setFilter(filter)
				.withImmediateFlush(false).setLayout(patternLayout).setName(name)
				.withPolicy(policy)
		...
		config.addAppender(fileOut);

		Map<String, LoggerConfig> logsMap = config.getLoggers();
		List<LoggerConfig> loggerConfigs = new ArrayList<>();
		for (LoggerConfig lc : logsMap.values()) {
			if (null != lc) {
				lc.stop();
				config.removeLogger(lc.getName());
				List<AppenderRef> list = new ArrayList<>();
				if (null != appenderMap) {
					for (Map.Entry<String, Appender> e : appenderMap.entrySet()) {
						String k = e.getKey();
						list.add(AppenderRef.createAppenderRef(k, lc.getLevel(), null));
					}
				}
				AppenderRef[] refs = list.toArray(new AppenderRef[list.size()]);
				LoggerConfig loggerConfig = LoggerConfig.createLogger(lc.isAdditive(), lc.getLevel(), lc.getName(), lc.isIncludeLocation() + "", refs, lc.getPropertyArray(), config, lc.getFilter());
				loggerConfigs.add(loggerConfig);
			}
		}

		for (LoggerConfig lc : loggerConfigs) {
			config.addLogger(lc.getName(), lc);
		}
