...
import com.oimchat.client.platform.fx.work.common.box.UserHeadBox;
 ...
 */
public class UserHeadImageHandler extends BaseHeadImageHandler<UserHeadBox> {

	public UserHeadImageHandler(AppContext appContext) {
		super(appContext);
	}
	...
	public URL getHeadUrl(String head) {
		...
		URL url = hr.getUserHeadPath(head);
		if (null == url) {
			url = hr.getUserHeadPath("1");
		}
		return url;
	}

	@Override
	public UserHeadBox getBox() {
		UserHeadBox box = this.appContext.getObject(UserHeadBox.class);
		return box;
	}
}
