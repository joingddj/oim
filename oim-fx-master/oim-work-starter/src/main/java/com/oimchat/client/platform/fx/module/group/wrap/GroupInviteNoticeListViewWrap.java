...
import com.oimchat.client.general.kernel.work.module.group.controller.GroupInviteApplyController;
...
public class GroupInviteNoticeListViewWrap extends AbstractMaterial {
	...
	public GroupInviteNoticeListViewWrap(AppContext appContext) {
		super(appContext);
		initConfig();
	}
	...
	private void loadList(int number) {
		...
		GroupInviteApplyController controller = appContext.getObject(GroupInviteApplyController.class);
	...
	private void reject(GroupInviteApply apply) {
		this.handle(GroupInviteApply.verify_handle_type_reject, apply);
	}

	private void accept(GroupInviteApply apply) {
		this.handle(GroupInviteApply.verify_handle_type_accept, apply);
	}

	private void handle(String verifyHandleType, GroupInviteApply apply) {
		...
		GroupInviteVerifyHandleData handle = new GroupInviteVerifyHandleData();
		GroupInviteHandler contactController = appContext.getObject(GroupInviteHandler.class);
		handle.setVerifyHandleType(verifyHandleType);
		handle.setApplyIds(applyIds);
		handle.setVerifyUserId(ownerUserId);

		contactController.inviteVerifyHandle(handle, (info) -> {
