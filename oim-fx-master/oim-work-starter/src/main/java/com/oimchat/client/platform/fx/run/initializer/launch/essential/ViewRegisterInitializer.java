...
public class ViewRegisterInitializer implements LaunchInitializer {

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
	public void initialize(AppContext appContext) {
		Set<Class<?>> classSet = ClassScaner.scan("com.oimchat.client.platform.fx.view.impl.classics");
		for (Class<?> c : classSet) {
			if (View.class.isAssignableFrom(c) && !c.isInterface()) {
				Class<?>[] interfaces = c.getInterfaces();

				for (Class i : interfaces) {
					if (i != View.class) {
						appContext.register(i, c);
					}
				}
			}
		}
